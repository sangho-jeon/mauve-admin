    ...
    <Background sender={props.showModal} onClick={props.click}>
      {isData(questionnaire) &&
        <ModalContainer>
          ...
          <Question>2. 회원 목표: <Answer>{questionnaire.goal}</Answer></Question>
          <Question>3. 월경 주기: <Answer>{questionnaire.menstrual_cycle}</Answer></Question>
          <Question>4. 임신 여부: <Answer>{getChildBirth(questionnaire.childbirth.has)}</Answer></Question>
          ...
          <Question>8. 식사 횟수: <Answer>{questionnaire.number_of_eating}</Answer></Question>
          <Question>9. 커피 섭취 여부: <Answer>{questionnaire.number_of_coffee}</Answer></Question>
          <Question>10. 질병 정보: <Answer>{questionnaire.diseases.text}</Answer></Question>
        </ModalContainer>
      };
    </Background>
  );
